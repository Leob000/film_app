...
class FiLMedNet(nn.Module):
    def __init__(
        ...
        self.num_cond_maps = (
            2 * self.module_dim if self.condition_method == "concat" else 0
        )
        self.fwd_count = 0
        ...
        module_H = feature_dim[1] // (
            stem_stride**stem_num_layers
        )  # Rough calc: work for main cases
        module_W = feature_dim[2] // (
            stem_stride**stem_num_layers
        )  # Rough calc: work for main cases
        self.coords = coord_map((module_H, module_W), self.device)
        # self.default_weight = Variable(torch.ones(1, 1, self.module_dim)).type(torch.cuda.FloatTensor)
        self.default_weight = torch.ones(
            1, 1, self.module_dim, device=self.device, dtype=torch.float
        )
        # self.default_bias = Variable(torch.zeros(1, 1, self.module_dim)).type(
        #     torch.cuda.FloatTensor
        # )
        self.default_bias = torch.zeros(
            1, 1, self.module_dim, device=self.device, dtype=torch.float
        )

        # Initialize stem
        stem_feature_dim = (
            feature_dim[0] + self.stem_use_coords * self.num_extra_channels
        )
        ...
        for fn_num in range(self.num_modules):
            with_cond = self.condition_pattern[
                self.module_num_layers * fn_num : self.module_num_layers * (fn_num + 1)
            ]
            mod = self.block(
                module_dim,
    ...
    def forward(self, x, film, save_activations=False):
        ...
        if self.condition_method == "concat":
            ...
            cond_maps = (
                cond_params.unsqueeze(3)
                .unsqueeze(4)
                .expand(cond_params.size() + x.size()[-2:])
            )
        else:
        ...
        # module_inputs = Variable(
        #     torch.zeros(feats.size())
        #     .unsqueeze(1)
        #     .expand(N, self.num_modules, self.module_dim, H, W)
        # ).type(torch.cuda.FloatTensor)
        module_inputs = (
            torch.zeros(feats.size(), device=self.device, dtype=torch.float)
            .unsqueeze(1)
            .expand(N, self.num_modules, self.module_dim, H, W)
        )
        ...
        for fn_num in range(self.num_modules):
            ...
            if fn_num == (self.num_modules - 1):
                ...
                module_inputs_updated[:, fn_num + 1] = (
                    module_inputs_updated[:, fn_num + 1] + layer_output
                )
    ...
    def __init__(
        ...
        if self.with_batchnorm:
            self.bn1 = nn.BatchNorm2d(
                out_dim, affine=((not self.with_cond[0]) or self.batchnorm_affine)
            )
        ...
        if (
            self.condition_method == "relu-film"
            or self.condition_method == "block-output-film"
...
def coord_map(shape, device, start=-1, end=1):
    ...
    x_coord_row = torch.linspace(start, end, steps=n, device=device, dtype=torch.float)
    y_coord_row = torch.linspace(start, end, steps=m, device=device, dtype=torch.float)
